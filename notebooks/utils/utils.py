...
def get_evaluation_predictions(
    weave_client: weave.trace.weave_client.WeaveClient, eval_call_id: str
) -> pd.DataFrame:
    """
    ...
    for eval_call in eval_calls.children():
        if (
            eval_call.op_name.split("/")[-1].split(":")[0]
...
def calculate_cohen_kappa(df: pd.DataFrame, labels: list) -> float:
    ...
    """
    required_cols = ["scores", "pred_score"]
    ...
    return cohen_kappa_score(score, df["pred_score"], labels=labels, weights="linear")
...
def calculate_kendall_tau(df: pd.DataFrame) -> float:
    ...
    return kendalltau(df["score"], df["pred_score"]).statistic
